...
export default function POSPage() {
  ...
  // Print functions
  // POS print — uses printInvoice utility (bold black, thermal printer optimized, with logo)
  const printPOS = () => {
    if (invoiceData) {
      // Map POSPage field names → printInvoice expected field names
      const data = {
        ...
        payment_status: invoiceData.due > 0 ? (invoiceData.paid > 0 ? 'partial' : 'due') : 'paid',
        subtotal: invoiceData.subtotal,
        vat_amount: invoiceData.vat_total,
        discount_amount: invoiceData.discount,
        total_amount: invoiceData.total,
        paid_amount: invoiceData.paid,
        due_amount: invoiceData.due,
        notes: invoiceData.notes || '',
        sold_by_name: invoiceData.sold_by_name || '',
  ...
  return (
    <>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[95vh] overflow-y-auto"
            ...
            <div className="invoice-print-area p-10">
              ...
              <div className="text-center mb-6">
                ...
                {/* Dynamic Invoice Logo from company config */}
                {(() => {
                  const c = invoiceData.company || company;
                  const showLogo = c?.show_invoice_logo !== 0 && c?.show_invoice_logo !== false;
                  const logoUrl = c?.invoice_logo_url || c?.logo_url || '';
                  const logoPos = c?.invoice_logo_position || 'center';
                  const logoW = c?.invoice_logo_width || 120;
                  const logoH = c?.invoice_logo_height || 60;
                  if (showLogo && logoUrl) {
                    return (
                      <div style={{ textAlign: logoPos as any, marginBottom: 8 }}>
                        <img src={logoUrl} alt="Logo"
                          style={{ width: logoW, height: logoH, objectFit: 'contain', display: 'inline-block' }}
                ...
                <h1 style={{ fontSize: '1.8rem', fontWeight: 900, color: '#111', fontFamily: 'Barlow Condensed, sans-serif', marginBottom: 2 }}>
                  {(invoiceData.company || company)?.shop_name || 'Wholesale Shop'}
                </h1>
      ...
      {/* POS MAIN LAYOUT */}
      <div className="flex flex-col md:flex-row md:h-[calc(100vh-112px)] md:gap-5 md:-m-6 md:p-6 -m-3 h-[calc(100vh-96px)]">

        {/* Mobile Tab Switcher */}
        {isMobile && (
          <div className="flex border-b bg-white flex-shrink-0">
            <button onClick={() => setMobileView('products')}
              className={`flex-1 py-2.5 text-sm font-bold transition-all ${mobileView === 'products' ? 'border-b-2 text-red-600' : 'text-gray-400'}`}
              style={mobileView === 'products' ? { borderColor: 'var(--primary, #dc2626)', color: 'var(--primary, #dc2626)' } : {}}>
              📦 Products
            </button>
            <button onClick={() => setMobileView('cart')}
              className={`flex-1 py-2.5 text-sm font-bold transition-all relative ${mobileView === 'cart' ? 'border-b-2 text-red-600' : 'text-gray-400'}`}
              style={mobileView === 'cart' ? { borderColor: 'var(--primary, #dc2626)', color: 'var(--primary, #dc2626)' } : {}}>
              🛒 Cart
              {cart.length > 0 && (
                <span className="absolute -top-0.5 right-1/4 w-5 h-5 rounded-full text-white text-xs font-bold flex items-center justify-center"
        ...
        {/* Products Section */}
        <div className={`flex-1 flex flex-col min-w-0 ${isMobile && mobileView !== 'products' ? 'hidden' : ''}`}
          style={isMobile ? { padding: '8px' } : {}}>
          <div className="mb-3 md:mb-4 flex gap-2 md:gap-3">
            <input ref={searchRef} type="text" value={search} onChange={e => setSearch(e.target.value)}
              className="power-input flex-1 text-sm md:text-base py-2 md:py-2.5" placeholder="🔍 Search product..." />
            {!isOnline && (
              <div className="flex items-center gap-1.5 bg-orange-50 text-orange-700 text-xs font-semibold px-2 md:px-3 rounded-lg border border-orange-200">
                📴
              </div>
            )}
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
            <div className="grid grid-cols-2 md:grid-cols-2 xl:grid-cols-3 gap-2 md:gap-3 pb-2">
              {filtered.map(product => (
                <div key={product.id} onClick={() => { addToCart(product); if (isMobile) setMobileView('cart'); }}
                  ...
                  <div className="flex items-start justify-between mb-1 md:mb-2">
                    ...
                    <span className={`text-xs font-bold px-1.5 md:px-2 py-0.5 rounded-full ${Number(product.total_stock) > 10 ? 'badge-green' : Number(product.total_stock) > 0 ? 'badge-yellow' : 'badge-red'
                      }`}>
                      {Number(product.total_stock) > 0 ? `${product.total_stock} ${product.unit || 'pcs'}` : 'Out'}
                    </span>
                  </div>
                  <p className="text-xs md:text-sm font-bold text-gray-900 leading-snug mb-0.5 line-clamp-2">{product.product_name}</p>
                  <p className="text-xs text-gray-400 mb-1 md:mb-2 truncate">{product.brand} {product.model && `· ${product.model}`}</p>
                  <div className="flex items-center justify-between">
                    <p className="text-xs md:text-sm font-black" style={{ color: 'var(--primary, #dc2626)' }}>৳{Number(product.sale_price).toLocaleString()}</p>
                    {Number(product.vat_percentage) > 0 && (
                      <span className="text-xs text-gray-400 hidden md:inline">+{product.vat_percentage}% VAT</span>
                    )}
                  </div>
                  {Number(product.batch_count) > 1 && (
                    <p className="text-xs text-blue-500 mt-1 font-semibold hidden md:block">⚡ {product.batch_count} batches</p>
                  )}
                </div>
              ))}
              {filtered.length === 0 && (
                <div className="col-span-2 md:col-span-3 flex flex-col items-center justify-center h-32 md:h-48 text-gray-300">
          ...
          {isMobile && mobileView === 'products' && cart.length > 0 && (
            <button onClick={() => setMobileView('cart')}
              className="fixed bottom-4 right-4 z-30 flex items-center gap-2 px-5 py-3 rounded-full shadow-lg text-white font-bold text-sm"
              style={{ background: 'var(--primary, #dc2626)' }}>
              🛒 View Cart ({cart.reduce((s, i) => s + i.quantity, 0)}) — ৳{total.toFixed(0)}
            </button>
          )}
        </div>

        {/* Cart Section */}
        <div className={`${isMobile ? 'flex-1' : 'w-80 xl:w-96'} flex flex-col bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm ${isMobile && mobileView !== 'cart' ? 'hidden' : ''}`}>
