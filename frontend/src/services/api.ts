...
api.interceptors.request.use((config) => {
  ...
  // NOTE: No X-Outlet-Id header needed. Backend middleware handles outlet scoping
  // based on the user's outlet_users assignment, not headers.
  return config;
});
...
export const stockAPI = {
  ...
  getProductStock: () => api.get('/stock/summary/stock'),
};
...
export const depositsAPI = {
  getToday: () => api.get('/deposits/today'),
  create: (data: any) => api.post('/deposits', data),
  update: (id: number, data: any) => api.put(`/deposits/${id}`, data),
